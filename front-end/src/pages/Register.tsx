...
const SocialButton: React.FC<{ provider: 'google' | 'facebook' }> = ({ provider }) => {
    ...
    const label = isGoogle
        ? (lang === 'ar' ? 'تسجيل الدخول عبر Google (قريباً)' : lang === 'de' ? 'Mit Google anmelden (Demnächst)' : 'Sign in with Google (Coming Soon)')
        : (lang === 'ar' ? 'تسجيل الدخول عبر Facebook (قريباً)' : lang === 'de' ? 'Mit Facebook anmelden (Demnächst)' : 'Sign in with Facebook (Coming Soon)');
    const tooltip = lang === 'ar' ? 'تسجيل الدخول عبر منصات التواصل غير متاح حالياً' : lang === 'de' ? 'Social Login ist derzeit nicht verfügbar' : 'Social login is currently unavailable';
    const bg = isGoogle
        ? 'bg-white dark:bg-slate-800/80 text-slate-500 dark:text-slate-400 border border-slate-300 dark:border-slate-700'
        : 'bg-[#1877F2]/40 text-white/70 border border-[#1877F2]/30';
    ...
    return (
        <button
            ...
            aria-label={label}
            title={tooltip}
            className={`w-full flex items-center justify-center gap-3 py-3 px-4 rounded-lg font-semibold cursor-not-allowed opacity-60 transition-all ${bg}`}
        >
            {isGoogle ? (
                ...
                <svg viewBox="0 0 24 24" className="w-5 h-5 fill-white opacity-60" aria-hidden="true">
...
const Register: React.FC = () => {
    ...
    const errorMessages: Record<string, Record<string, string>> = {
        ...
        min12Chars: { en: 'Min. 12 characters', de: 'Mind. 12 Zeichen', ar: '١٢ حرفاً على الأقل' },
        uppercase: { en: 'Uppercase letter', de: 'Großbuchstabe', ar: 'حرف كبير' },
        number: { en: 'Number', de: 'Zahl', ar: 'رقم' },
        specialChar: { en: 'Special character', de: 'Sonderzeichen', ar: 'رمز خاص' },
        passwordsMatch: { en: 'Passwords match', de: 'Passwörter stimmen überein', ar: 'كلمات المرور متطابقة' },
        passwordsMismatch: { en: 'Passwords do not match', de: 'Passwörter stimmen nicht überein', ar: 'كلمات المرور غير متطابقة' },
    };
    ...
    const checks = {
        length: formData.password.length >= 12 && formData.password.length <= 128,
                                ...
                                {formData.password.length > 0 && (
                                    <div className="mt-3 space-y-2">
                                        ...
                                        <div className="grid grid-cols-2 gap-1">
                                            {[
                                                { ok: checks.length, label: t('min12Chars') },
                                                { ok: checks.upper, label: t('uppercase') },
                                                { ok: checks.number, label: t('number') },
                                                { ok: checks.special, label: t('specialChar') },
                                            ].map(req => (
                                ...
                                {formData.confirmPassword.length > 0 && (
                                    <div className={`flex items-center gap-1.5 mt-2 text-[10px] ${formData.password === formData.confirmPassword ? 'text-emerald-400' : 'text-red-400'}`}>
                                        {formData.password === formData.confirmPassword
                                            ...
                                            : <><XCircle className="w-3 h-3" />{t('passwordsMismatch')}</>}
                                    </div>
                                )}
                            </div>
                        </div>
