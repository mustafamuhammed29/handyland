...
import { X, Cpu, Activity, Clock, ShieldCheck, ChevronRight } from 'lucide-react';
    ...
    return (
        <div className="fixed inset-0 z-[140] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-in fade-in duration-300">
            <div
                ...
                role="dialog"
                aria-modal="true"
                aria-labelledby="repair-device-title"
                tabIndex={-1}
                className="relative w-full max-w-4xl bg-slate-900 border-4 border-blue-500/30 rounded-3xl overflow-hidden shadow-2xl shadow-blue-900/50 flex flex-col md:flex-row max-h-[85vh]"
            >
                {/* Grid Overlay */}
                ...
                <div className="w-full md:w-2/3 p-8 overflow-y-auto bg-slate-950/95 relative custom-scrollbar">
                    <div className="flex justify-between items-center mb-8">
                        <div>
                            <h4 id="repair-device-title" className="text-2xl font-black text-white tracking-tight">{t('repair.title', 'Available Services')}</h4>
                    ...
                    <div className="space-y-3">
                        {(selectedDevice.services || []).map((service, idx) => {
                            const serviceTitle = service.label ||
                                (service.type === 'screen' ? t('repair.screenReplacement', 'Displayreparatur') :
                                 ...
                                 service.type);
                            const priceLabel = service.price && service.price > 0 ? `${service.price}€` : t('repair.na', 'On Request');

                            return (
                                <button
                                    key={idx}
                                    type="button"
                                    aria-label={`${serviceTitle}, ${priceLabel}`}
                                    onClick={() => handleBookRepair(selectedDevice.model || 'Unknown Device', serviceTitle, service.price)}
                                    className="w-full text-left group relative bg-slate-900/40 backdrop-blur-sm border border-slate-800/60 hover:border-blue-500/40 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:border-blue-500 rounded-2xl p-4 transition-all duration-300 hover:bg-slate-800/60 cursor-pointer overflow-hidden shadow-sm hover:shadow-[0_0_30px_rgba(59,130,246,0.1)]"
                                >
