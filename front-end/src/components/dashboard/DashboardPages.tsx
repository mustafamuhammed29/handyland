...
export const DashboardPages: React.FC = () => {
    ...
    const fetchPages = async () => {
        try {
            ...
            addToast(t('pages.error.load', 'Failed to load pages'), 'error');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            addToast(t('pages.success.update', 'Page updated successfully'), 'success');
        } catch (error) {
            console.error('Failed to update page', error);
            addToast(t('pages.error.update', 'Failed to update page'), 'error');
        }
    };

    if (loading) return <div className="text-white">{t('common.loading', 'Loading pages...')}</div>;

    return (
        <div className="space-y-6">
            <h2 className="text-2xl font-bold text-white flex items-center gap-2">
                <FileText className="w-6 h-6 text-blue-400" />
                {t('pages.title', 'Page Management')}
            </h2>

            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {/* Sidebar - Page List */}
                <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-4 h-fit">
                    <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-4">{t('pages.select', 'Select Page')}</h3>
                ...
                <div className="md:col-span-3">
                    {selectedPage ? (
                        <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
                            <div className="flex justify-between items-center mb-6">
                                <div>
                                    ...
                                    <p className="text-slate-400 text-sm">{t('pages.lastUpdated', 'Last updated')}: {new Date(selectedPage.lastUpdated).toLocaleString()}</p>
                                </div>
                                <div className="flex gap-2">
                                    <a
                                        ...
                                        title={t('pages.viewLive', 'View Live Page')}
                                    >
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label htmlFor="pageContent" className="block text-sm font-medium text-slate-400 mb-2">
                                        {t('pages.contentLabel', 'Content (HTML/Markdown supported)')}
                                    </label>
                                    <textarea
                                        id="pageContent"
                                        value={content}
                                        ...
                                        placeholder={t('pages.contentPlaceholder', 'Enter page content...')}
                                    />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-12 text-center">
                            ...
                            <h3 className="text-xl font-bold text-white mb-2">{t('pages.noSelection', 'No Page Selected')}</h3>
                            <p className="text-slate-400">{t('pages.noSelectionDesc', 'Select a page from the list to edit its content.')}</p>
                        </div>
                    )}
                </div>
            </div>
