 ...
 * Modal dialog for creating a new physical warehouse location (Phase 2B).
 */
...
export const AddLocationModal: React.FC<AddLocationModalProps> = ({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!trimmedCode) {
            setFormError('كود الموقع إلزامي');
            return;
        }

        if (!trimmedZone) {
            setFormError('المنطقة (Zone) إلزامية');
            return;
        }
        ...
        try {
            ...
            if (res.data?.success) {
                toast.success('تمت إضافة موقع التخزين بنجاح');
                onSuccess();
                onClose();
            } else {
                setFormError(res.data?.message || 'فشل إنشاء الموقع');
            }
        } catch (err: any) {
            const errorCode = err.response?.data?.error;
            if (errorCode === 'WAREHOUSE_LOCATION_CODE_EXISTS') {
                setFormError('كود الموقع مستخدم مسبقاً في المستودع. يرجى اختيار كود فريد.');
            } else {
                setFormError(err.response?.data?.message || 'حدث خطأ أثناء إنشاء الموقع. تحقق من المدخلات.');
            }
        } finally {
            setIsSubmitting(false);
        }
            ...
            <div className="bg-slate-900 border border-slate-700/80 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-950/60">
                    <div className="flex items-center gap-2.5">
                        ...
                        <div>
                            <h3 className="text-base font-bold text-white">إضافة موقع تخزين جديد</h3>
                            <p className="text-xs text-slate-400">إنشاء موقع فيزيائي لقطع الصيانة</p>
                        </div>
                    </div>
                    <button
                        type="button"
                ...
                <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-4 custom-scrollbar">
                    ...
                    <div>
                        <label className="block text-xs font-semibold text-slate-300 mb-1.5">
                            كود الموقع (Location Code) <span className="text-red-400">*</span>
                        </label>
                        <input
                            type="text"
                            placeholder="مثال: A-01-S02-B04"
                            value={locationCode}
                        ...
                        />
                        <p className="text-[11px] text-slate-500 mt-1">كود الموقع غير قابل للتغيير بعد الإنشاء.</p>
                    </div>
                    ...
                    <div>
                        ...
                        <input
                            type="text"
                            placeholder="مثال: Zone A - الشاشات"
                            value={zone}
                    ...
                    <div>
                        ...
                        <input
                            type="text"
                            placeholder="ملاحظات تنظيمية حول موقع التخزين..."
                            value={description}
