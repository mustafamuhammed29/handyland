                        ...
                        <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md p-6">
                            ...
                            <form onSubmit={handleLendPhone} className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div><label className="text-xs text-slate-400 mb-1 block">Frist (Rückgabe am) <span className="text-red-400">*</span></label><input type="date" required value={lendFormData.dueDate} onChange={e => setLendFormData({ ...lendFormData, dueDate: e.target.value })} className="w-full bg-slate-800 border-slate-700 rounded-xl px-4 py-2 text-white" /></div>
                                </div>
                                <div><label className="text-xs text-slate-400 mb-1 block">Übergabeprotokoll (Mängel, Kratzer etc.)</label><textarea value={lendFormData.notes} onChange={e => setLendFormData({ ...lendFormData, notes: e.target.value })} rows={2} className="w-full bg-slate-800 border-slate-700 rounded-xl px-4 py-2 text-white" /></div>
